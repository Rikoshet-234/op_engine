...
#ifdef MORE_SPAM
			Msg("CALifeStoryRegistry::add, adding Story item ID [%u], Object [%s] at level [%s]",id, object->name_replace(), *ai().game_graph().header().level(ai().game_graph().vertex(object->m_tGraphID)->level_id()).name());
#endif

	ALife::STORY_P_PAIR_IT	I = m_objects.find(id);
	if (I != m_objects.end()) {
		R_ASSERT2			(no_assert,shared_str().sprintf("Story object ID=%u is already in the Story registry!",id).c_str());
		return;
	} 
