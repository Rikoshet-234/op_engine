...
void CGameMtlLibrary::Load()
{
	...
	bool mtrlLogShow=OPFuncs::Dumper->isParamSet(OPFuncs::ExpandedCmdParams::KnownParams::dMaterialsLoad);
	...
	if (OBJ) {
		...
		for (IReader* O = OBJ->open_chunk_iterator(count); O; O = OBJ->open_chunk_iterator(count,O)) {
			...
			if (mtrlLogShow)
				Msg("CGameMtlLibrary::Load, Loaded material info %s %i",*M->m_Name,M->GetID());
		}
		OBJ->close		();
	}
	...
	if (OBJ){
		...
		for (IReader* O = OBJ->open_chunk_iterator(count); O; O = OBJ->open_chunk_iterator(count,O)) {
			...
			if (mtrlLogShow)
				Msg("Loaded material info: %s",M->dbg_Name());
	...
	for (GameMtlPairIt p_it=material_pairs.begin(); material_pairs.end() != p_it; ++p_it){
		SGameMtlPair* S	= *p_it;
		if (mtrlLogShow)
			Msg("Material pair prepare: %i",S->GetID());
		int mtl0_idx=GetMaterialIdx(S->mtl0);
		int mtl1_idx=GetMaterialIdx(S->mtl1);
		int idx0		= mtl0_idx*material_count+mtl1_idx;
		int idx1		= mtl1_idx*material_count+mtl0_idx;
