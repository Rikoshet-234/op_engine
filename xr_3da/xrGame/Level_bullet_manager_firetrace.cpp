...
#	else
							...
							stalker                 = smart_cast<CAI_Stalker*>(initiator);//! jarni: reuse variable
							if (stalker)
...
//	Fvector			hit_normal;
	...
	if (E.bullet.flags.allow_sendhit && !E.Repeated)
	{
		...
		Hit.ammoSection		= E.bullet.m_ammoSect;
		NET_Packet			np;
		Hit.Write_Packet	(np);
		
		//Msg("Hit sended: %d[%d,%d] ammoSect:[%s]", Hit.whoID, Hit.weaponID, Hit.BulletID,Hit.ammoSection.c_str());
...
//	if(shoot_factor<RICOCHET_THRESHOLD &&  )
	if (((f+shoot_factor)<ricoshet_factor) && bullet->flags.allow_ricochet)	{
		...
		energy_lost = 1.f - bullet->speed/old_speed;//! jarni: reuse variable
