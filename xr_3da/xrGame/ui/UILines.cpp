...
std::string CUILines::GetLongestLineText()
{
	std::string result;
	if (GetLinesCount()>1)
	{
		std::for_each(m_lines.begin(),m_lines.end(),[&](CUILine line)
		{
			if (line.GetSize()>int(result.size()))
			{
				std::string lineStr;
				for (u32 i=0; i<line.m_subLines.size(); i++)
				{
					lineStr+=line.m_subLines[i].m_text.c_str();
				}
				result=lineStr;
			}
		});
