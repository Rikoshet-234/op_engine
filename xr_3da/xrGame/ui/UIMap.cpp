...
void CUILevelMap::Init	(shared_str name, CInifile& gameLtx, LPCSTR sh_name)
{
	...
	if (OPFuncs::Dumper->isParamSet(OPFuncs::ExpandedCmdParams::KnownParams::dMapLoadErrors))
	{
		...
		if(FALSE==fsimilar(kw,kh,EPS_L)){
			Msg("CUILevelMap::Init, --incorrect global rect definition for map [%s]  kw=%f kh=%f",*MapName(),kw,kh);
			Msg("CUILevelMap::Init, --try x2=%f or  y2=%f",m_GlobalRect.x1+kh*BoundRect().width(), m_GlobalRect.y1+kw*BoundRect().height());
		}
	}
