		...
		for(;it!=m_GameMaps.end();++it){
			...
			for(it2=it; it2!=m_GameMaps.end();++it2){
				...
				if(l->GlobalRect().intersected(l2->GlobalRect())){
					Msg("CUIMapWnd::Init, --error-incorrect map definition global rect of map [%s] intersects with [%s]", *l->MapName(), *l2->MapName());
				}
			}
			if(FALSE == l->GlobalRect().intersected(GlobalMap()->BoundRect())){
				Msg("CUIMapWnd::Init, --error-incorrect map definition map [%s] places outside global map", *l->MapName());
			}
...
bool CUIMapWnd::OnMouse(float x, float y, EUIMessages mouse_action)
{
	...
	Fvector2 cursor_pos2 = GetUICursor()->GetCursorPosition();

	if(GlobalMap() && !GlobalMap()->Locked() && ActiveMapRect().in( cursor_pos2 ) ){
		switch (mouse_action)
		{
