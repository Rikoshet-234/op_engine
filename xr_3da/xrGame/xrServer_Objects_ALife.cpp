...
LPCSTR CSE_ALifeDynamicObjectVisual::get_visual_name()
{
	return *visual_name;
}
void CSE_ALifeDynamicObjectVisual::set_visual_name(LPCSTR _visual_name)
{
	set_visual(_visual_name);
}
...
void CSE_ALifeObjectHangingLamp::on_render(CDUInterface* du, ISE_AbstractLEOwner* owner2, bool bSelected, const Fmatrix& parent,int priority, bool strictB2F)
{
	inherited1::on_render		(du, owner2,bSelected,parent,priority,strictB2F);
	if ((1==priority)&&(false==strictB2F)){
		u32 clr					= bSelected?0x00FFFFFF:0x00FFFF00;
		Fmatrix main_xform, ambient_xform;
		owner2->get_bone_xform		(*light_main_bone,main_xform);
		main_xform.mulA_43			(parent);
		if(flags.is(flPointAmbient) ){
			owner2->get_bone_xform	(*light_ambient_bone,ambient_xform);
