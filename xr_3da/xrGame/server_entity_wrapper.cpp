...
{
	bool slrLogShow=OPFuncs::Dumper->isParamSet(OPFuncs::ExpandedCmdParams::dSpawnRegistryLoad);
	NET_Packet				net_packet;
	u16						ID;
	IReader					*chunk;
	...
	if (slrLogShow)
		Msg("CServerEntityWrapper::load, Try read spawn registry for section %s",s_name);
	...
	m_object->UPDATE_Read	(net_packet);
	if (slrLogShow)
		Msg("Done read spawn registry for %s",m_object->name_replace());
}
