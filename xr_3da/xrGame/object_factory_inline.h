...
IC	const CObjectItemAbstract &CObjectFactory::item	(const CLASS_ID &clsid) const
{
	...
	string16 text_id;CLSID2TEXT(clsid,text_id);
	VERIFY3				((I != clsids().end()) && ((*I)->clsid() == clsid),"CObjectFactory::item, item not found in ObjectFactory",text_id);
	return				(**I);
}
#else
...
IC	void CObjectFactory::add	(CObjectItemAbstract *item)
{
	...
	if (!(I == clsids().end()))
	{
		...
		Msg("CObjectFactory::add, Already registered clsid in factory %s",temp);
	}
	VERIFY				(I == clsids().end());
	...
	if (!(I == clsids().end()))
		Msg("CObjectFactory::add, Already registered script_clsid in factory %s",item->script_clsid().c_str());
	VERIFY				(I == clsids().end());
#endif
