...
void CStateMonsterRestFunAbstract::execute()
{
	...
	if ((dist < object->db().m_fDistToCorpse + 0.5f) && (time_last_hit + MIN_DELAY < Device.dwTimeGlobal)) {
		...
		if  (target && target->m_pPhysicsShell) {
			Fvector			dir2;
			dir2.add			(Fvector().sub(target->Position(), object->Position()), object->Direction());
			
			float			h,p;
			dir2.getHP		(h,p);
			dir2.setHP		(h, p + 5 * PI / 180);
			dir2.normalize	();
			
			// выполнить бросок
			for (u32 i=0; i<target->m_pPhysicsShell->Elements().size();i++) {
				target->m_pPhysicsShell->Elements()[i]->applyImpulse(dir2, IMPULSE_TO_CORPSE * target->m_pPhysicsShell->getMass() / target->m_pPhysicsShell->Elements().size());
			}
