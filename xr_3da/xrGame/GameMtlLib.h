...
public:
	...
	{
		GameMtlIt it=GetMaterialItByID(ID);
		VERIFY3(materials.end() != it,"Material ID not found in gamemtr.xr.",std::to_string(ID).c_str()); return (u16)(it-materials.begin());
	}
	IC u16				GetMaterialIdx	(LPCSTR name)	
	{
		GameMtlIt it=GetMaterialIt(name);
		VERIFY3(materials.end() != it,"Material name not found in gamemtr.xr.",name); return (u16)(it-materials.begin());
	}
