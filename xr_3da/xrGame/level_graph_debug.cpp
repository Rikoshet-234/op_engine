...
void CLevelGraph::draw_stalkers		(const int &vertex_id)
{
	...
	typedef OBJECT_REGISTRY::_const_iterator		const_iterator_ot;
	...
	if (show_text) {
		...
		const_iterator_ot			I = objects.objects().begin();
		const_iterator_ot			E = objects.objects().end();
		for (; I != E; ++I) {
...
//			font.OutNext		("%s",stalker->name_replace());
			...
			Fvector				position2 = convert_position(graph.vertex(stalker->m_tGraphID)->game_point());
			render.draw_aabb	(position2,radius,radius,radius,color);
			first_time			= false;
			continue;
		}
	}
...
void CLevelGraph::draw_objects		(const int &vertex_id)
{
	...
	typedef CALifeGraphRegistry::OBJECT_REGISTRY	OBJECT_REGISTRY;
	typedef OBJECT_REGISTRY::_const_iterator		const_iterator_or;
	typedef CALifeMonsterDetailPathManager::PATH	PATH;
	const OBJECT_REGISTRY		&objects = ai().alife().graph().objects()[vertex_id].objects();
	...
	if (show_text) {
		bool					first_time = true;
		const_iterator_or			I = objects.objects().begin();
		const_iterator_or			E = objects.objects().end();
		for (; I != E; ++I) {
			CSE_ALifeDynamicObject	*object = (*I).second;
...
//			font.OutNext		("%s",monster->name_replace());
			...
			Fvector				position2 = convert_position(graph.vertex(monster->m_tGraphID)->game_point());
			render.draw_aabb	(position2,radius,radius,radius,color);
			first_time			= false;
			continue;
		}
