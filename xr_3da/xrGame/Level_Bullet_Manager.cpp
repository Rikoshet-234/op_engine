...
bool CBulletManager::CalcBullet (collide::rq_results & r_rq_storage, xr_vector<ISpatial*>& /*rq_spatial*/, SBullet* bullet, u32 delta_time)
{
	VERIFY					(bullet);
	...
	result							= Level().ObjectSpace.RayQuery(r_rq_storage, RD, firetrace_callback, &bullet_data, test_callback, NULL);
	
	if (result && bullet_data.bStopTracing) 
	{
		range						= (r_rq_storage.r_begin()+r_rq_storage.r_count()-1)->range;
	}
