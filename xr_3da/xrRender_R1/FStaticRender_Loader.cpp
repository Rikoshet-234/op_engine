...
void CRender::level_Load(IReader *fs)
{
	...
	// Shaders
	g_pGamePersistent->LoadTitle		("st_loading_shaders", "Loading shaders (R1)...");
	{
	...
	if	(!g_dedicated_server)	{
		// VB,IB,SWI
		g_pGamePersistent->LoadTitle("st_loading_geometry", "Loading geometry (R1)...");
		...
		// Visuals
		g_pGamePersistent->LoadTitle("st_loading_spatial_db", "Loading visuals (R1)...");
		chunk						= fs->open_chunk(fsL_VISUALS);
		LoadVisuals					(chunk);
		chunk->close				();

		// Details
		g_pGamePersistent->LoadTitle("st_loading_details", "Loading details (R1)...");
		Details->Load				();
	}
	
	// Sectors
	g_pGamePersistent->LoadTitle("st_loading_sectors_portals", "Loading sectors (R1)...");
	LoadSectors					(fs);
