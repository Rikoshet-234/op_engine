...
void FLOD::Load			(LPCSTR N, IReader *data, u32 dwFlags)
{
	...
	for (int f=0; f<8; f++)
	{
		...
		Fvector					N2,T;
		N2.set					(0,0,0);
		T.mknormal				(v[0].v,v[1].v,v[2].v);	N2.add	(T);
		T.mknormal				(v[1].v,v[2].v,v[3].v);	N2.add	(T);
		T.mknormal				(v[2].v,v[3].v,v[0].v);	N2.add	(T);
		T.mknormal				(v[3].v,v[0].v,v[1].v);	N2.add	(T);
		N2.div					(4.f);
		facets[f].N.normalize	(N2);
		facets[f].N.invert		();
	}
