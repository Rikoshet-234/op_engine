...
public:
	...
	IC void dbg_DrawAABB			(Fvector& iT, float sx, float sy, float sz, u32 iC)
	{
		Fvector half_dim;	
		half_dim.set(sx,sy,sz); 
		Fmatrix	TM;	
		TM.translate(iT); 
		dbg_DrawOBB(TM,half_dim,iC);
	}
	void dbg_DrawOBB				(Fmatrix& T, Fvector& half_dim, u32 C);
	IC void dbg_DrawTRI				(Fmatrix& iT, Fvector* p, u32 iC)											{	dbg_DrawTRI(iT,p[0],p[1],p[2],iC);	}
