...
void CDrawUtilities::DrawEntity(u32 clr, ref_shader s)
{
    ...
    {
        // fill VB
        FVF::LIT*	pv2	 = (FVF::LIT*)Stream->Lock(6,vs_LIT->vb_stride,vBase);
        pv2->set		(0.f,1.f,0.f,clr,0.f,0.f);	pv2++;
        pv2->set		(0.f,1.f,.5f,clr,1.f,0.f);	pv2++;
        pv2->set		(0.f,.5f,.5f,clr,1.f,1.f);	pv2++;
        pv2->set		(0.f,.5f,0.f,clr,0.f,1.f);	pv2++;
        pv2->set		(0.f,.5f,.5f,clr,1.f,1.f);	pv2++;
        pv2->set		(0.f,1.f,.5f,clr,1.f,0.f);	pv2++;
...
void CDrawUtilities::DrawFlag(const Fvector& p, float heading, float height, float sz, float sz_fl, u32 clr, BOOL bDrawEntity){
    ...
    if (bDrawEntity){
		...
		FVF::L*	pv2	 	= (FVF::L*)Stream->Lock(6,vs_L->vb_stride,vBase);
        sz				*= 0.8f;
        pv2->set			(p.x,p.y+height,p.z,clr);											pv2++;
        pv2->set			(p.x+rx*sz,p.y+height,p.z+rz*sz,clr);                               pv2++;
        sz				*= 0.5f;
        pv2->set			(p.x,p.y+height*(1.f-sz_fl*.5f),p.z,clr);                           pv2++;
        pv2->set			(p.x+rx*sz*0.6f,p.y+height*(1.f-sz_fl*.5f),p.z+rz*sz*0.75f,clr);   	pv2++;
        pv2->set			(p.x,p.y+height*(1.f-sz_fl),p.z,clr);                               pv2++;
        pv2->set			(p.x+rx*sz,p.y+height*(1.f-sz_fl),p.z+rz*sz,clr);                   pv2++;
		Stream->Unlock	(6,vs_L->vb_stride);
		// and Render it as line list
    	DU_DRAW_DP		(D3DPT_LINELIST,vs_L,vBase,3);
    }else{
	    ...
	    pv2->set			(p.x,p.y+height*(1.f-sz_fl),p.z,clr); 								pv2++;
    	pv2->set			(p.x,p.y+height,p.z,clr); 											pv2++;
	    pv2->set			(p.x+_sin(heading)*sz,((pv2-2)->p.y+(pv2-1)->p.y)/2,p.z+_cos(heading)*sz,clr); pv2++;
    	pv2->set			(*(pv2-3)); 														pv2++;
	    pv2->set			(*(pv2-2)); 														pv2++;
    	pv2->set			(*(pv2-4)); 														pv2++;
		Stream->Unlock	(6,vs_L->vb_stride);
