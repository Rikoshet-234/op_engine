...
public:
	...
	{
		VERIFY		(p && count);
		//VERIFY2		(B.count + count < NET_PacketSizeLimit,shared_str().sprintf("Oversized netpacket before write! [%i > %i]",B.count+count,NET_PacketSizeLimit).c_str());
	...
	IC void w_stringZ			( shared_str& p, bool iniString = false )
	{
		if (*p)	w(*p,(u32)xr_strlen(p)+1,iniString);
		else	w_u8(0);
	}
	...
	{
		//VERIFY2		(pos < B.count,shared_str().sprintf("Seek position went out of the buffer boundaries. pos=%i B.count=%i",pos,B.count).c_str());
	...
	IC void		r				( void* p, u32 count)
	{
		...
		r_pos		+= count;
		//VERIFY2		(r_pos<=B.count,shared_str().sprintf("Read position went out of the buffer boundaries. r_pos=%i B.count=%i",r_pos,B.count).c_str());
		if (r_pos>B.count)
		{
