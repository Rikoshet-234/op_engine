...
//	if file already exist - update info
	...
	while (temp[0]) 
	{
		...
		if (!exist(path))	
		{
			...
			desc.modif			= u32(-1);
			std::pair<files_it,bool> I2 = files.insert(desc); 
...
void CLocatorAPI::ProcessArchive(LPCSTR _path, LPCSTR base_path)
{
	...
	{
		Msg("CLocatorAPI::_initialize, ignoring file %s", _path); // добавлено alpet: предотвращает exception при распакованной gamedata
		return;
	}
	...
	R_ASSERT							(A.size>0);
	if (OPFuncs::Dumper->isParamSet(OPFuncs::ExpandedCmdParams::KnownParams::dFileSystem))
...
*/
		CHECK_OR_EXIT	(pFSltx,make_string("CLocatorAPI::_initialize,Cannot open file \"%s\".\nCheck your working folder.",fs_ltx));
		// append all pathes    
...
#endif // DEBUG
			CHECK_OR_EXIT		(I.second,"CLocatorAPI::_initialize, the file 'fsgame.ltx' is corrupted (it contains duplicated lines).\nPlease reinstall the game or fix the problem manually.");
		}
		r_close			(pFSltx);
	...
	Msg				("CLocatorAPI::_initialize, FS: %d files cached, %dKb memory used.",files.size(),(M2-M1)/1024);
	...
	//show fs
	if (OPFuncs::Dumper->isParamSet(OPFuncs::ExpandedCmdParams::KnownParams::dFileSystemVars))
	{
		Msg("CLocatorAPI::_initialize, Dump filesystem variables begin");
		for				(PathPairIt p_it=pathes.begin(); p_it!=pathes.end(); p_it++)
		{
			char* id	= LPSTR(p_it->first);
			char* path	= LPSTR(p_it->second->m_Path);
			Msg("%s %s",id,path);
		}
		Msg("CLocatorAPI::_initialize, Dump filesystem variables end");
	}
	//
...
bool CLocatorAPI::check_for_file	(LPCSTR path, LPCSTR _fname, string_path& fname, const file *&desc)
{
	...
	if (I == files.end())
	{
		Msg("WARNING: CLocatorAPI::check_for_file file not found file %s in files list (size = %d) ", desc_f.name, files.size () );		
		return				(false);
	}
