	...
	for (int i=0; i<p_count; i++, bit<<=1)
	{
		if (test_mask&bit) {
			...
			if (_abs(cls)>=r) test_mask&=~bit;			// fully - no need to test this plane
				...
				EFC_Visible	er	= AABB_OverlapPlane(planes[i],mM);
				if (fcvFully==er)	test_mask&=~bit;					// fully - no need to test this plane
				else if (fcvNone==er){ test_mask=0; return fcvNone;	}	// none - return
			}
		}
	}
	return test_mask ? fcvPartial:fcvFully;
