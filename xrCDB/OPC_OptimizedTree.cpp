...
void AABBNoLeafTree::Store(IWriter& writer)
{
	writer.w_u32(mNbNodes);
	writer.w(mNodes, mNbNodes*sizeof(AABBNoLeafNode));
}

bool AABBNoLeafTree::Restore(IReader& reader)
{
	mNbNodes = reader.r_u32();
	mNodes = xr_alloc<AABBNoLeafNode>(mNbNodes);
	CHECKALLOC(mNodes);
	ZeroMemory(mNodes,mNbNodes*sizeof(AABBNoLeafNode));
	reader.r(mNodes, mNbNodes*sizeof(AABBNoLeafNode));
	uintptr_t oldbase = 0, newbase = (uintptr_t)mNodes;
	if (!(mNodes[0].mData&1))// Positive non-leaf node
	{
		oldbase = mNodes[0].mData - sizeof(AABBNoLeafNode);
	}
	else if (!(mNodes[0].mData2&1))// Negative non-leaf node
	{
		oldbase = mNodes[0].mData - sizeof(AABBNoLeafNode);
	}
	else
	{
		R_ASSERT2(false, "Both nodes of tree are leaves");
	}

	for (udword i = 0; i < mNbNodes; ++i)
	{
		AABBNoLeafNode& node = mNodes[i];
		if (!(node.mData&1))// Positive non-leaf node
		{
			node.mData = newbase + (node.mData - oldbase);
		}
		if (!(node.mData2&1))// Negative non-leaf node
		{
			node.mData2 = newbase + (node.mData2 - oldbase);
		}
	}
	return true;
}
